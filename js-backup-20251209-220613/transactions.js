...
import { showToast, formatCurrency, formatDate, getCategoryIcon, today } from './utils.js';
import { getElements, openTransactionModal, showElement, hideElement } from './ui.js';
...
export async function handleTransactionSubmit(e) {
    ...
    if (transactionData.type === 'Receita') {
        ...
        if (!transactionData.paymentDate) {
            transactionData.paymentDate = transactionData.dueDate;
        }
...
export function handleEditTransaction(transaction) {
    ...
    elements.transactionForm.creditCard.value = transaction.creditCard || '';

    // Import and call UI functions
    import('./ui.js').then(ui => {
        ui.togglePaymentDateVisibility();
        ui.toggleExpenseTypeVisibility();
        ui.toggleCreditCardVisibility();
 ...
 * Handle mark as paid
 */
export async function handleMarkAsPaid(id) {
    try {
        const docRef = doc(getTransactionsCollectionRef(), id);
 ...
 */
export function renderGenericTransactionList(element, transactions, showEmptyStateEl) {
    element.innerHTML = '';
            ...
            <div class="flex items-center mb-2 sm:mb-0 flex-grow mr-4">
                ...
                <div class="flex-1 min-w-0">
                    <p class="text-base font-medium text-gray-900 dark:text-gray-100 truncate ${isOverdue ? 'text-red-600 dark:text-red-400 font-bold' : ''}">
                        ...
                        ${t.creditCard ? `<span class="text-xs text-purple-500 ml-1">(${t.creditCard})</span>` : ''}
                    </p>
